...
class ProjectGui(pycam.Gui.BaseUI):
    ...
    def __init__(self, event_manager):
        ...
        # TODO: Re-implement actions for GTK 4
        ...
        # the settings window
        # TODO: Re-add these when buttons are implemented
        ...
        try:
            ...
            self.preferences_window.connect("close-request", self.toggle_preferences_window, False)
        ...
        if self.about_window:
            self.about_window.connect("close-request", self.toggle_about_window, False)
        # menu bar - UIManager removed in GTK 4
        # TODO: Implement new menu system for GTK 4
        ...
        def add_main_window_item(item, name, **extra_args):
            # GTK 4: Use append instead of pack_start
            if main_window:
                main_window.append(item)
        ...
        def disable_gui():
            # TODO: Re-implement menubar sensitivity for GTK 4
        ...
        for auto_control_name, location_control_name, browse_button, key in (
            ...
            try:
                auto_control = self.gui.get_object(auto_control_name)
                location_control = self.gui.get_object(location_control_name)
                browse_control = self.gui.get_object(browse_button)
                
                if auto_control:
                    auto_control.connect("clicked", self._locate_external_program, key)
                if location_control:
                    self.settings.add_item("external_program_%s" % key, location_control.get_text,
                                           location_control.set_text)
                if browse_control:
                    browse_control.connect("clicked", self._browse_external_program_location, key)
            except AttributeError:
                # External program UI elements not found in minimal UI
                pass
        for objname, callback in (
            ...
            try:
                ...
                if obj:
                    obj.connect("activate", callback)
            except AttributeError:
                # Workspace UI elements not found in minimal UI
                pass
        # set the icons (in different sizes) for all windows
        # Gtk.window_set_default_icon_list(*get_icons_pixbuffers()) FIXME
        # GTK 4: Old menu system replaced with MenuManager
        # load menu data
        ...
        if self.recent_manager is not None:
            ...
            try:
                recent_menu = self.gui.get_object("OpenRecentModel")
                if recent_menu:
                    recent_menu.set_visible(False)
            except AttributeError:
                # Recent model menu not found in minimal UI
                pass
        # GTK 4: Menubar handled by MenuManager
        # load the menubar and connect functions to its items
        # self.menubar = uimanager.get_widget("/MenuBar")
        self.menubar = None  # TODO: Get menubar reference from MenuManager if needed
        # GTK 4: Legacy menu system replaced with MenuManager
        # dict of all merge-ids
        # menu_merges = {}

        def clear_menu(menu_key):
            # GTK 4: Menu clearing handled by MenuManager
            pass

        def append_menu_item(menu_key, base_path, widget, name):
            # GTK 4: Menu items handled by MenuManager
            pass
        ...
        # GTK 4: These UI registrations handled by MenuManager
    ...
    def configure_drag_and_drop(self, obj):
        # TODO: Implement GTK 4 drag-and-drop
        return  # FIXME
